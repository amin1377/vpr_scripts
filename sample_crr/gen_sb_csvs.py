...
def assign_opin_wire(n_opin, wire_sinks, fan_out, rng):
    """
    OPIN-to-wire connections (no IPIN sinks).  Each OPIN's `fan_out`
    connections are distributed evenly across all four directions
    (Left, Right, Top, Bottom).  Connections within each direction are
    balanced by current fan-in.
    ...
    # Pre-group sink indices by direction
    dir_indices = {d: [] for d in directions}
    for idx, (direction, _lane) in enumerate(wire_sinks):
        dir_indices[direction].append(idx)

    n_wire_sink = len(wire_sinks)
    base = fan_out // n_dirs      # connections per direction (floor)
    extras = fan_out % n_dirs     # first `extras` dirs get one more

    wire_fanin  = [0] * n_wire_sink
    opin_to_sinks = []

    for opin_idx in range(n_opin):
        # Rotate which directions receive the extra connection each round
        dir_order = directions[opin_idx % n_dirs:] + directions[:opin_idx % n_dirs]
        chosen = []
        for i, d in enumerate(dir_order):
            ...
            if len(cands) < count:
                raise ValueError(
                    f'Direction {d} has only {len(cands)} wire sinks, '
                    f'need {count} for fan_out_opin={fan_out}')
            cands_sorted = sorted(cands, key=lambda j: (wire_fanin[j], rng.random()))
            chosen.extend(cands_sorted[:count])
...
def build_csv(filepath, W, ipin_cols, opin_rows,
    ...
    ow_opin_to_sink, _ = assign_opin_wire(
        n_opin, wire_sinks, fan_out_opin, rng)
